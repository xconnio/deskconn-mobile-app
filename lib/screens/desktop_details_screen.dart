...
class DesktopDetailsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(desktop['name'] ?? 'Desktop'),
      ),
      body: ListView(
        ...
        children: [
          ...
          _ActionTile(
            ...
            title: "Volume",
            onTap: () => _setVolume(session, desktop),
          ),
          _ActionTile(
            icon: Icons.lock,
            title: "Lock Screen",
            onTap: () => _lock(session, desktop),
          ),
        ],
      ),
    );
  ...
  Future<void> _setBrightness(
    ...
    await session.session!.call(
      "io.xconn.desktop.brightness.set",
      args: [desktop['authid'], 50],
    );
  }
  ...
  Future<void> _setVolume(
    ...
    await session.session!.call(
      "io.xconn.desktop.volume.set",
      args: [desktop['authid'], 30],
    );
  }
